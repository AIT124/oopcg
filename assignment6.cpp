//write c++ program using STL for sortimg and searchimg user defined record such as personal record(NAme .dob,telephone number) using vecor container
#include<iostream>
#include<vector>//container header file
#include<algorithm>//to have access to algorithm function like sort(), merge ()
using namespace std;
class Personal
{
	string name,dob,tel_no;
	public:
		Personal(string name1,string dob1,string tel):name(name1),dob(dob1),tel_no(tel){ }//initalising the variable in the constucture out beacue for constant function it can change
		string getName(){ return name; }//function
		string getDob(){ return dob; }//function
		string getTelNo(){return tel_no;}//funtion
};
...
int main()
{
	vector<Personal> pvec;//create a vector of type personal
	int cho;
	char ch;
	...
	do
	{
		...
		fflush(stdin);//flush the out put buffer
		cin>>ch;
	}while(ch=='Y'||ch=='y');
return 0;
}